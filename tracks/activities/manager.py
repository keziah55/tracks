...
class ActivityManager(QObject):
    ...
    def _initialise_activity(self, activity: Activity) -> ActivityObjects:
        ...
        df = self._load_activity_df(activity)
        ...
        plot = PlotWidget(
            data, activity, style=plot_style, months=month_range, y_series=y_series
        )
    ...
    def _load_activity_df(self, activity: Activity) -> pl.DataFrame:
    ...
    def _activity_csv_file(self, activity: Activity, raise_not_exist=False):
        ...
        if raise_not_exist and not filepath.exists():
            msg = f"csv file for activity '{activity.name}' not found"
