...
def get_op_test_params():
    params = [(Divide, 15, 5, 3.0), (Divide_time_min, 15, 5, 0.05)]
    return params
...
class TestActivities:
    ...
    def test_create_activity(self):
        ...
        with pytest.warns(UserWarning):
            activity.add_measure(
                ...
                sig_figs=3,
                relation=Relation(
                    activity.steps, activity.time, Divide_time_min, "Step rate"
                ),
                cmp_func=float,
            )
    ...
    def test_filter_measures(self, demo_activity):
        filtered = demo_activity.filter_measures(
            "show_unit", lambda value: value is False
        )
        expected = ["time", "steps"]
